...
// Start command handler with a welcome message and command instructions in creating SEO-optimised meta description for blog posts. Provide me with keywords for your meta caption, and I'll save the results directly to your Notion page
bot.start((ctx) => {
...
function sendCommandList(ctx) {
    ...
    const commandListMessage = "🆘 Available commands:\n" + commandList.join("\n");
...
bot.action('send_to_notion', async (ctx) => {
    ...
    ctx.replyWithHTML('Login to your notion', Markup.inlineKeyboard([
        Markup.button.callback('login to notion', loginUrl)]))
});

// Define a variable to store the original user input (keywords) and also to store the button click count in bot memory to prevent spam
...
async function processAIResponse(ctx) {
    try {
        ...
        if (!response) {
            ctx.reply("Sorry, I couldn't generate content for you. Please try again.");
        }
        else {
            const formattedResponse = `<b style="color:green;">Voila 🪄 Your greeting is ready!</b>\n\n${response.content}`;
            ctx.replyWithHTML(formattedResponse, Markup.inlineKeyboard([
                Markup.button.callback("👎 I don't like it. Give me another one!", 'generate_new')
            ]));
        }
    } catch (error) {
...
bot.action('generate_new', async (ctx) => {
    ...
    // track to many regenerate on the bad keyword input
    if (!buttonClickCount[userId]) {
        buttonClickCount[userId] = 0;
    }

    const tipsMessage =
        `<b style="color:green;">Sorry, I'm struggling 😓 to come up with a greeting you like</b>\n
    ...
    try {
        ...
        ctx.replyWithHTML(formattedNewResponse, Markup.inlineKeyboard([
            Markup.button.callback("👎 I don't like it. Give me another one!", 'generate_new')
        ]));
    } catch (error) {
